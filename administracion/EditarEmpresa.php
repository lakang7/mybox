...
<title>GAAG Archivos / Editar Empresa</title>
<?php
	$con=Conexion();
	$sql_consulta="select * from empresa where idempresa=".$_GET["id"];
	$result_consulta=pg_exec($con,$sql_consulta);
	$empresa=pg_fetch_array($result_consulta,0);
?>
</head>
		...
		<div class="row">
			...
			<div class="col-md-8" style="">
				...
				<div class="col-md-10" style="border-bottom: 1px solid #E9E9E9; height: auto; padding-bottom: 30px; padding-top: 15px;">																				
					<form role="form" id="form_CreacionEmpresa" method="post" action="../recursos/clases/acciones.php?action=3">
  						...
  						<div class="form-group">
    						...
    						<input type="input" value="<?php echo $empresa[1] ?>" maxlength="130" class="form-control" name="nombre_empresa" id="nombre_empresa" placeholder="Ingrese el nombre de la empresa" required="required">
  						</div>
  						<div class="form-group">
	    					<label for="rfc_empresa" style="font-family: 'PT Sans Narrow', sans-serif;">RFC</label>
    						<input type="input" value="<?php echo $empresa[2] ?>" maxlength="30" class="form-control" name="rfc_empresa" id="rfc_empresa" placeholder="Ingrese el rfc de la empresa" required="required">
  						</div>
